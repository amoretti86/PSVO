...
class trainer:
    ...
    def set_placeholders(self, x_0_idx, obs, hidden):
        self.x_0_idx = x_0_idx
        self.obs = obs
        self.hidden = hidden
    ...
    def evaluate_R_square(self, MSE_ks, y_means, y_vars, hidden_set, obs_set, test=False):
    ...
    def train(self, obs_train, obs_test, print_freq, hidden_train, hidden_test):
        ...
        log_ZSMC_train, log_train = self.SMC_train.get_log_ZSMC(self.obs, self.hidden)
        ...
        MSE_test, R_square_test = self.evaluate_R_square(MSE_ks_train, y_means_train, y_vars_train,
                                                         hidden_test, obs_test, test=False)
        ...
        for i in range(self.epoch):
            ...
            for j in range(0, len(obs_train), self.batch_size):
                self.sess.run(train_op, feed_dict={self.obs: obs_train[j:j + self.batch_size],
                                                   self.x_0_idx: np.arange(j, j + self.batch_size),
