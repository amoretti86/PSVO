...
def main(_):
    ...
    q_train_layers = [int(x) for x in FLAGS.q_AF_network_layers.split(",")]
    f_train_layers = [int(x) for x in FLAGS.f_AF_network_layers.split(",")]
    g_train_layers = [int(x) for x in FLAGS.g_network_layers.split(",")]
    ...
    smoothing_perc = tf.placeholder(tf.float32, name="hidden")

    # transformations
    q_train_tran = NF(flow_layers,
                      Dx,
                      hidden_layers=q_train_layers,
    ...
    if use_bootstrap:
        ...
        f_train_tran = NF(flow_layers,
                          Dx,
                          hidden_layers=f_train_layers,
                          sample_num=sample_num,
                          flow_type="MAF",
                          name="f_flow")
