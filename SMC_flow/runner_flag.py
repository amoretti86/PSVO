...
# flow architectures
flow_layers = 2
q_AF_network_layers = [10]
f_AF_network_layers = [10]
g_network_layers = [10]
...
q_AF_network_layers = ",".join([str(x) for x in q_AF_network_layers])
f_AF_network_layers = ",".join([str(x) for x in f_AF_network_layers])
g_network_layers = ",".join([str(x) for x in g_network_layers])
...
flags.DEFINE_integer("flow_layers", flow_layers, "number of flow (MAF/IAF) applied")
flags.DEFINE_string("q_AF_network_layers", q_AF_network_layers, "architecture for q's MAF/IAF, int seperated by comma, "
                                                                "for example: '50,50' ")
flags.DEFINE_string("f_AF_network_layers", f_AF_network_layers, "architecture for q's MAF/IAF, int seperated by comma, "
                                                                "for example: '50,50' ")
flags.DEFINE_string("g_network_layers", g_network_layers, "architecture for g network, int seperated by comma, "
                                                          "for example: '50,50' ")
...
flags.DEFINE_boolean("use_2_q", use_2_q, "if q uses two networks q1(x_t|x_t-1) and q2(x_t|y_t), "
                                         ...
                                         "q_takes_y as False, "
                                         "q_uses_true_X as False")
flags.DEFINE_integer("sample_num", sample_num, "number of samples used to evaluate flow's max likelihood point")
