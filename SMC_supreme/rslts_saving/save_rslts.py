...
def plot_learning_results(RLT_DIR, Xs_val, hidden_train, max_fig_num = 20):
    ...
    for i in range(n_train):
        ax.plot(np.average(Xs_val[i,:,:,0], axis=1), np.average(Xs_val[i,:,:,1], axis=1), np.average(Xs_val[i,:,:,2], axis=1))
        ax.scatter(np.average(Xs_val[i,0,:,0]), np.average(Xs_val[i,0,:,1]), np.average(Xs_val[i,0,:,2]))
    plt.savefig(RLT_DIR+"/3D_All_X_Paths/")
