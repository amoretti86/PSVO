...
if __name__ == '__main__':
	...
	with tf.Session() as sess:
		...
		Xs = log[0]
		Xs_val = np.zeros((time, n_particles, Dx))
		for i, X in enumerate(Xs):
			X_val = sess.run(X, feed_dict = {obs:obs_train[0]})
			Xs_val[i] = X_val
	...
	plt.figure()
	plt.plot(Xs_val[:, :, 0], alpha = 0.01, c = 'black')
	plt.plot(hidden_train[0][:, 0], c = 'yellow')
	sns.despine()
	if store_res == True:
	  plt.savefig(RLT_DIR + "hidden states")
	plt.show()
