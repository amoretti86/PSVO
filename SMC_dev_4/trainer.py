...
class trainer:
                ...
                if self.save_res:
                    ...
                    if save_gradient:
                        ...
                        for gradients_dict in self.list_of_gradients_dict:
                            ...
                            with open(
                                    self.epoch_data_DIR + "gradient_{}.p".format(i + 1), "wb") as f:
                                print("Dump gradients")
                                print(gradients_val_dict)
    ...
    def evaluate_gradients(self, gradients_dict, feed_dict):
        ...
        #gradients_val_samples = self.evaluate(gradients, feed_dict, average=True)
        #gradients_val = [gradients_val_samples[i] for i in range(len(gradients))]
        gradients_val_samples = [self.evaluate(gradients, feed_dict, average=True) for _ in range(100)]
