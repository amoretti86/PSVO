...
class trainer:
    ...
    def set_epoch_data_saving(self, RLT_DIR, save_trajectory, save_y_hat, save_gradient, saving_num, SNR_sample_num,
                              SNR_NP_list, SNR_collect_grads_points):
        self.save_res = True
        ...
        if self.save_gradient:
            ...
            self.SNR_NP_list = SNR_NP_list
            self.SNR_collect_grads_points = SNR_collect_grads_points
    ...
    def train(self,
        ...
        # n_particles = 1
        log_ZSMC, log, grad_sampling_loss = self.SMC.get_log_ZSMC(self.obs, self.hidden, self.Input)
        print("build log_ZSMC for n_particles = 1")
        ...
        #TODO: add evaluate gradient
        list_of_log_ZSMC_np = [log_ZSMC]

        # when np=1, there is no resampling loss
        list_of_sampling_loss_np = []

        list_of_total_loss_np = []
        if self.save_gradient:
            for n_particles in self.SNR_NP_list[1:]:
                print("build log_ZSMC for n_particles = ", n_particles)
                log_ZSMC_np, log, grad_sampling_loss_np = self.SMC.get_log_ZSMC(self.obs, self.hidden, self.Input, n_particles=n_particles)
                #total_loss_np = log_ZSMC_np + grad_sampling_loss_np

                list_of_log_ZSMC_np.append(log_ZSMC_np)
                list_of_sampling_loss_np.append(grad_sampling_loss_np)
                #list_of_total_loss_np.append(total_loss_np)

        #self.list_of_gradients_dict = self.set_up_gradient(list_of_log_ZSMC_np)

        self.list_of_gradients_sampling_dict = self.set_up_gradient(list_of_sampling_loss_np)

        #self.list_of_gradients_total_dict = self.set_up_gradient(list_of_total_loss_np)
                    ...
                    if self.save_gradient:
                        ...
                        if self.save_count_down > 0:
                            ...
                            for gradients_dict in self.list_of_gradients_dict:
                                gradients_val_dict = self.evaluate_gradients(gradients_dict, gradients_feed_dict)

                                n_particles = self.SNR_NP_list[np_idx]
                                with open(self.epoch_data_DIR + "gradient_{}_np_{}.p".format(i + 1, n_particles), "wb") as f:
                            ...
                            np_idx = 1
                            for gradients_sampling_dict in self.list_of_gradients_sampling_dict:
                                gradients_sampling_val_dict = \
                                    self.evaluate_gradients(gradients_sampling_dict, gradients_feed_dict)

                                n_particles = self.SNR_NP_list[np_idx]
                                with open(self.epoch_data_DIR + "gradient_sampling_{}_np_{}.p".format(i + 1, n_particles),
                                          "wb") as f:
                                    print("Dump gradients_sampling for epoch {} np {}".format(i + 1, n_particles))
                                    pickle.dump(gradients_sampling_val_dict, f)

                                np_idx += 1

                            """
                            np_idx = 1
                            for gradients_total_dict in self.list_of_gradients_total_dict:
                                gradients_total_val_dict = \
                                    self.evaluate_gradients(gradients_total_dict, gradients_feed_dict)

                                n_particles = self.SNR_NP_list[np_idx]
                                with open(
                                        self.epoch_data_DIR + "gradient_total_{}_np_{}.p".format(i + 1, n_particles),
                                        "wb") as f:
                                    print("Dump gradients_total for epoch {} np {}".format(i + 1, n_particles))
                                    pickle.dump(gradients_total_val_dict, f)
