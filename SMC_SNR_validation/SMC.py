...
class SMC:
    ...
    def get_log_ZSMC(self, obs, hidden, Input, q_cov=1, n_particles=None):
        """
        ...
        with tf.variable_scope(self.name):
            ...
            for t in range(0, time):
                ...
                log_W = tf.add(f_t_log_prob, g_t_log_prob - q_t_log_prob, name="log_W_{}".format(t))  # (n_particles, batch_size)
                ...
                if n_particles > 1:
                    resample_idx = self.get_resample_idx(log_W, t) # log_W shape (n_particles, batch_size)
                    X_ancestor = tf.gather_nd(X, resample_idx)                    # (n_particles, batch_size, Dx)

                    recover_idx = resample_idx[:,:,0]
                    list_of_recover_idx.append(recover_idx)
                else:
                    X_ancestor = X
                X_prev = X
            ...
            if n_particles > 1:
                list_of_recover_idx = tf.stack(list_of_recover_idx) # (time, n_particles, batch_size)
                list_of_recover_idx = tf.transpose(list_of_recover_idx, perm=[1,2,0], name="list_of_recover_idx") # (n_particles, batch_size, time)

                grad_sampling_step = []

                logit_log_Ws = log_Ws[:, 0:time-1, :]
                for particle_idx in range(n_particles):
                    recover_idx_per_particle = list_of_recover_idx[particle_idx] # (batch_size, time)
                    ...
                    grad_sampling_step.append(grad_sampling_per_particle)

                grad_sampling_step = tf.stack(grad_sampling_step)  # (n_particles, batch_size, time)

                grad_sampling_loss = tf.multiply(tf.reduce_mean(grad_sampling_step), log_ZSMC, name='grad_sampling_loss')

            else:
                grad_sampling_loss = 0
        ...
        return log_ZSMC, log, grad_sampling_loss
    ...
    def get_resample_idx(self, log_W, t):
        ...
        idx = tf.where(idx >= n_particles, final_fixup, idx)  # (n_particles, batch_size)
