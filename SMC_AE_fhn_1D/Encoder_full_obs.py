...
class Encoder_full_obs:
	...
	def get_mvn(self, x_prev_NpxNbxDz, t):
		with tf.variable_scope(self.scope + '/get_mvn', reuse=tf.AUTO_REUSE):
			...
			self.sigma_cons = tf.get_variable("sigma_cons",
											  shape = [1],
											  dtype = tf.float32,
											  initializer = tf.constant_initializer(self.sigma_init),
											  trainable = False)
			self.sigma = tf.eye(self.x_dim) * tf.maximum(tf.nn.softplus(self.sigma_cons), self.sigma_cons_stablizer)
