...
class Encoder:
	def __init__(self, x_dim, n_particles, batch_size, time, scope = 'Encoder'):
		self.scope = scope
		self.x_dim = x_dim
		...
		self.Enc_layer_1_dim = 60
		self.Enc_layer_2_dim = 60
	...
	def encoding(self, YInput_NbxTxDy):
		with tf.variable_scope(self.scope + '/encoding'):
			Enc_layer_1 = fully_connected(YInput_NbxTxDy, self.Enc_layer_1_dim,
			...
			X_hat_NbxTxDz = fully_connected(Enc_layer_2, self.x_dim,
											weights_initializer=tf.orthogonal_initializer(),
											biases_initializer=tf.zeros_initializer(),
											activation_fn=None,
											reuse = tf.AUTO_REUSE, scope = "X_hat_NbxTxDz")
